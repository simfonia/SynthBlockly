...
### 2026-01-30: 監聽器管理與演奏效能
- **事件積木的生命週期**：發現點擊「執行」按鈕時，舊的事件回呼 (Function) 並未從 Serial/MIDI 監聽清單中移除。原因在於 HatBlockManager.reset() 僅清空了內部 Map 而未呼叫 unregisterById。修正後，每次執行前都會確實執行徹底註銷。
- **和弦演奏與 ADSR 限制**：針對範例 14 進行深度偵錯，確認當 Release 設定大於播放時值時，聲音長度感官上會失真。修正建議為調小 R 值，而非強行在代碼端截斷發音通道。
- **PolySynth 資源累積**：在高頻率觸發下，PolySynth 容易達到發音上限。已將預設 maxPolyphony 提高至 64，並移除 0.1s 延遲以提高系統吞吐量。
